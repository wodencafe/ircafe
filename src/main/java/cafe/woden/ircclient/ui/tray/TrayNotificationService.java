    ...
    private static boolean tryMacOsascript(String title, String body, String targetKey) {
        ...
        try {
            File appResourcesDir = new File(
                    new File(System.getProperty("jpackage.app-path")).getParentFile(),
                    "../Resources"
            ).getCanonicalFile();
            File notifierBin = new File(appResourcesDir, "terminal-notifier.app/Contents/MacOS/terminal-notifier");

            if (notifierBin.exists()) {
                String[] split_target = targetKey.split("\\|");
                String server = split_target[0];
                String channel = split_target[1];
                File iconFile = new File(appResourcesDir, "Ircafe.icns");
                String deepLink = "ircafe://focus/" + server + "/" + java.net.URLEncoder.encode(channel, StandardCharsets.UTF_8);

                ProcessBuilder pb = new ProcessBuilder(
                        notifierBin.getAbsolutePath(),
                        "-title", escapeApple(title),
                        "-message", escapeApple(body),
                        "-group", "cafe.woden.ircclient",
                        "-open", deepLink,
                        "-appIcon", iconFile.getAbsolutePath(),
                        "-activate", "cafe.woden.ircclient"
                );
