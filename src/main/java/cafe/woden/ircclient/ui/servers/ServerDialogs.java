...
public class ServerDialogs {
    ...
    runOnEdt(() -> {
      ...
      if (curOpt.isEmpty()) {
        JOptionPane.showMessageDialog(
            parent,
            "Server '" + id + "' is not a persisted server and can't be edited here.\n\n"
  ...
  public void openSaveEphemeralServer(Window parent, String serverId) {
    runOnEdt(() -> {
      ...
      Optional<IrcProperties.Server> ephOpt = (ephemeralServers == null) ? Optional.empty() : ephemeralServers.find(id);
      if (ephOpt.isEmpty()) {
        JOptionPane.showMessageDialog(
            parent,
      ...
      if (ephemeralServers != null
        ...
        JOptionPane.showMessageDialog(parent,
            "An ephemeral server with id '" + nextId + "' already exists.\n\n"
      ...
      serverRegistry.upsert(next);
      if (ephemeralServers != null) {
        // Remove the ephemeral copy (importers will also avoid re-adding if a persisted entry exists).
