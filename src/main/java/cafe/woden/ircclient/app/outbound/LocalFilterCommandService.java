 ...
 */
@Component
public class LocalFilterCommandService {
  ...
  private void printHelp(TargetRef out) {
    ui.appendStatus(out, "(filter)", "Filters hide lines from transcripts (they do not affect IRC traffic or logging). ");
    ...
    ui.appendStatus(out, "(filter)", "  /filter del <name-or-mask> [more...]  (mask supports * and ?, or re:/.../)");
    ui.appendStatus(out, "(filter)", "  /filter enable|disable|toggle [<name-or-mask> ...]  (no args = global; use @ for current buffer)");
    ...
    ui.appendStatus(out, "(filter)", "  /filter placeholders [on|off|toggle|default] [target=<scope>]");
    ui.appendStatus(out, "(filter)", "  /filter placeholders collapsed [on|off|toggle|default] [target=<scope>]");
    ui.appendStatus(out, "(filter)", "  /filter override list [format=table|cmd]");
    ui.appendStatus(out, "(filter)", "  /filter override set scope=<glob> filters=on|off|default placeholders=on|off|default collapsed=on|off|default");
  ...
  private void list(TargetRef out, String formatRaw) {
    ...
    for (int i = 0; i < rules.size(); i++) {
      ...
      if (r.action() != null && r.action() != FilterAction.HIDE) b.append(" action=").append(r.action());
      if (r.direction() != null && r.direction() != FilterDirection.ANY) b.append(" dir=").append(r.direction());
      ...
      if (r.hasTextRegex()) b.append(" text=/").append(r.textRegex().pattern()).append("/").append(flagsString(r.textRegex()));
  ...
  private void move(TargetRef out, FilterCommand.Move cmd) {
    ...
    if (name.isEmpty()) {
      ui.appendError(out, "(filter)", "Usage: /filter move <name> <pos|top|bottom|up [n]|down [n]|before <other>|after <other>>");
      return;
    }
    ...
    switch (mode) {
      ...
      case UP -> toIdx = Math.max(0, fromIdx - Math.max(1, Objects.requireNonNullElse(cmd.amount(), 1)));
      case DOWN -> toIdx = Math.min(size - 1, fromIdx + Math.max(1, Objects.requireNonNullElse(cmd.amount(), 1)));
      case TO -> {
      ...
      case BEFORE, AFTER -> {
        ...
        if (other.isEmpty()) {
          ui.appendError(out, "(filter)", "Usage: /filter move <name> " + (mode == FilterCommand.MoveMode.BEFORE ? "before" : "after") + " <other>");
          return;
        }
        ...
        if (otherIdx == fromIdx) {
          ui.appendStatus(out, "(filter)", "Rule '" + name + "' is already " + (mode == FilterCommand.MoveMode.BEFORE ? "before" : "after") + " itself.");
          return;
        }
    ...
    // No-op.
    if (toIdx == fromIdx) {
      ui.appendStatus(out, "(filter)", "Rule '" + rules.get(fromIdx).nameKey() + "' is already at position " + (fromIdx + 1) + ".");
      return;
    }
    ...
    ui.appendStatus(out, "(filter)", "Moved '" + moving.nameKey() + "' from " + (fromIdx + 1) + " to " + (insertIdx + 1) + " (of " + rules.size() + ").");
  }
  ...
  private void add(TargetRef out, String nameRaw, FilterCommand.FilterRulePatch patch) {
    ...
    if (rules.stream().anyMatch(r -> r != null && r.nameKey().equals(key))) {
      ui.appendError(out, "(filter)", "Rule already exists: '" + name + "' (use /filter set or /filter del)");
      return;
    }
  ...
  private void addReplace(TargetRef out, String nameRaw, FilterCommand.FilterRulePatch patch) {
    ...
    if (idx >= 0) {
      ...
      FilterRule replacement = new FilterRule(
          prev.id(),
          r.name(),
          r.enabled(),
          r.scopePattern(),
  ...
  private void rename(TargetRef out, String oldNameRaw, String newNameRaw) {
    ...
    if (rules.stream().anyMatch(r -> r != null && r.nameKey().equals(newKey)) && !oldName.toLowerCase(Locale.ROOT).equals(newKey)) {
    ...
    rules.set(idx, next);
    applyNewRules(cur, rules);
    ui.appendStatus(out, "(filter)", "Renamed filter rule: " + prev.nameKey() + " -> " + next.nameKey());
  }
  ...
  private void del(TargetRef out, List<String> namesOrMasksRaw) {
    List<String> specs = (namesOrMasksRaw == null) ? List.of() : namesOrMasksRaw.stream()
    ...
    if (specs.isEmpty()) {
      ui.appendStatus(out, "(filter)", "Usage: /filter del <name-or-mask> [more...] (masks support * and ?, or re:/.../)");
      return;
    }
    ...
    ui.appendStatus(out, "(filter)", "Deleted " + removed + " filter(s). Now have " + next.size() + ".");
  }
...
private void setEnabled(TargetRef out, List<String> namesOrMasksRaw, boolean enabled) {
  ...
  if (hasAt) {
    ...
    FilterCommand.ToggleMode mode = enabled ? FilterCommand.ToggleMode.OFF : FilterCommand.ToggleMode.ON;
  ...
  applyNewRules(cur, rules);
  ui.appendStatus(out, "(filter)", (enabled ? "Enabled " : "Disabled ") + changed + " filter(s).");
}
...
private void toggle(TargetRef out, List<String> namesOrMasksRaw) {
  ...
  private void applyNewRules(FilterSettings cur, List<FilterRule> nextRules) {
    ...
    try {
      ...
      if (rebuildService != null && active != null) {
        ...
        if (!ok) {
          ui.appendStatus(active, "(filter)", "Note: transcript rebuild skipped (no history available). New rules apply to new lines only.");
        }
      }
    } catch (Exception ignored) {
    }
  ...
  private static FilterRule buildRuleForAdd(String name, FilterCommand.FilterRulePatch patch) {
    ...
    RegexSpec text = patch.textSpecified() ? normalizeText(patch.textRegex()) : new RegexSpec("", EnumSet.noneOf(RegexFlag.class));
  ...
  private static FilterRule applyPatch(FilterRule prev, FilterCommand.FilterRulePatch patch) {
    ...
    boolean enabled = patch.enabledSpecified() ? Boolean.TRUE.equals(patch.enabled()) : prev.enabled();
  ...
  private static FilterRule copyWithEnabled(FilterRule r, boolean enabled) {
    ...
    return new FilterRule(r.id(), r.name(), enabled, r.scopePattern(), r.action(), r.direction(), r.kinds(), r.fromNickGlobs(), r.textRegex(), r.tags());
  }
  ...
  private List<String> buildExportLines(FilterSettings cur, String format) {
    String f = (format == null || format.isBlank()) ? "all" : format.trim().toLowerCase(Locale.ROOT);
    ...
    for (FilterScopeOverride o : cur.overrides()) {
      out.add("/filter override set scope=" + o.scopePattern()
          + " filters=" + tri(o.filtersEnabled())
          + " placeholders=" + tri(o.placeholdersEnabled())
          + " collapsed=" + tri(o.placeholdersCollapsed()));
    }
...
private void handleShow(TargetRef out, FilterCommand.Show cmd) {
  ...
  boolean currentFiltersEnabled = resolveCurrentFiltersEnabled(cmd, cur, active, scope);
    ...
    Boolean newFiltersEnabled = switch (cmd.mode()) {
  ...
  FilterSettings next = upsertOverride(cur, scope,
      newFiltersEnabled, true,
      null, false,
      null, false);
  ...
  ui.appendStatus(out, "(filter)", "Show filtered lines for " + scope + " => " + showFilteredLabel(newFiltersEnabled));
  if (had && !has && newFiltersEnabled == null) {
    ui.appendStatus(out, "(filter)", "(override cleared; now inheriting defaults)");
  }

  printEffectiveSummary(out, active);
  ui.appendStatus(out, "(filter)", "Runnable: /filter override set scope=" + scope
      + " filters=" + tri(newFiltersEnabled)
      + " placeholders=default collapsed=default");
}

private boolean resolveCurrentFiltersEnabled(FilterCommand.Show cmd, FilterSettings cur, TargetRef active, String scope) {
  try {
...
private void handlePlaceholders(TargetRef out, FilterCommand.Placeholders cmd) {
  ...
  boolean current = resolveCurrentPlaceholdersEnabled(cmd, cur, active, scope);
    ...
    // Use a switch *expression* to keep definite assignment simple for the compiler.
    Boolean newVal = switch (cmd.mode()) {
  ...
  boolean has = findOverride(next.overrides(), scope) != null;
  ui.appendStatus(out, "(filter)", "Filtered (N) placeholders for " + scope + " => " + tri(newVal));
  ...
  printEffectiveSummary(out, active);
  ui.appendStatus(out, "(filter)", "Runnable: /filter override set scope=" + scope
      + " placeholders=" + tri(newVal)
      + " filters=default collapsed=default");
}

private boolean resolveCurrentPlaceholdersEnabled(FilterCommand.Placeholders cmd, FilterSettings cur, TargetRef active, String scope) {
  try {
    if (cmd.scopePattern() == null) {
...
private void handlePlaceholdersCollapsed(TargetRef out, FilterCommand.PlaceholdersCollapsed cmd) {
  ...
  printEffectiveSummary(out, active);
  ui.appendStatus(out, "(filter)", "Runnable: /filter override set scope=" + scope
      + " collapsed=" + tri(newVal)
      + " filters=default placeholders=default");
}

private boolean resolveCurrentCollapsed(FilterCommand.PlaceholdersCollapsed cmd, FilterSettings cur, TargetRef active, String scope) {
  try {
    if (cmd.scopePattern() == null) {
...
private void handleDefaults(TargetRef out, FilterCommand.Defaults cmd) {
  ...
  boolean f = cmd.filtersSpecified() ? Boolean.TRUE.equals(cmd.filtersEnabledByDefault()) : cur.filtersEnabledByDefault();
  boolean p = cmd.placeholdersSpecified() ? Boolean.TRUE.equals(cmd.placeholdersEnabledByDefault()) : cur.placeholdersEnabledByDefault();
  boolean c = cmd.collapsedSpecified() ? Boolean.TRUE.equals(cmd.placeholdersCollapsedByDefault()) : cur.placeholdersCollapsedByDefault();
  int preview = cmd.previewSpecified()
...
private void listOverrides(TargetRef out, String formatRaw) {
  FilterSettings cur = safeSettings();
  String format = (formatRaw == null || formatRaw.isBlank()) ? "table" : formatRaw.trim().toLowerCase(Locale.ROOT);
  ...
  if (format.equals("cmd") || format.equals("commands")) {
    ...
    for (FilterScopeOverride o : cur.overrides()) {
      ui.appendStatus(out, "(filter)", "/filter override set scope=" + o.scopePattern()
          + " filters=" + tri(o.filtersEnabled())
          + " placeholders=" + tri(o.placeholdersEnabled())
          + " collapsed=" + tri(o.placeholdersCollapsed()));
    }
  ...
  ui.appendStatus(out, "(filter)", String.format("%-30s %-10s %-14s %-10s",
      "Scope", "Filters", "Placeholders", "Collapsed"));
  for (FilterScopeOverride o : cur.overrides()) {
    ui.appendStatus(out, "(filter)", String.format("%-30s %-10s %-14s %-10s",
        o.scopePattern(),
...
private void setOverride(TargetRef out, FilterCommand.OverrideSet cmd) {
  ...
  FilterSettings next = upsertOverride(cur, scope,
      fe, cmd.filtersSpecified(),
      pe, cmd.placeholdersSpecified(),
	  ...
	  ui.appendStatus(out, "(filter)", "Override " + (has ? "set" : "cleared") + " for " + scope
	      + (had && !has ? " (now inheriting defaults)" : ""));
	  printEffectiveSummary(out, active);
  ui.appendStatus(out, "(filter)", "Runnable: /filter override set scope=" + scope
      + " filters=" + tri(eff == null ? null : eff.filtersEnabled())
      + " placeholders=" + tri(eff == null ? null : eff.placeholdersEnabled())
      + " collapsed=" + tri(eff == null ? null : eff.placeholdersCollapsed()));
}
...
private void applySettings(
  ...
  if (defaultsChanged) {
    ...
    runtimeConfig.rememberFilterPlaceholdersCollapsedByDefault(next.placeholdersCollapsedByDefault());
  }
  if (previewChanged) {
  ...
  try {
	...
	private void rebuildMaybe(TargetRef out, TargetRef active) {
	  try {
	    ...
	    if (!rebuilt) {
	      ui.appendStatus(out, "(filter)", "(note: transcript rebuild skipped for this buffer; filter changes apply to new lines)");
	    }
	  } catch (Exception ignored) {
	  }
	}
	...
	private void printEffectiveSummary(TargetRef out, TargetRef target) {
	  try {
	    ...
	    ui.appendStatus(out, "(filter)", "  filtering=" + onOff(fe.value()) + sourceSuffix(fe)
	        + " (show-filtered=" + onOff(!fe.value()) + ")");
...
private static FilterScopeOverride findOverride(List<FilterScopeOverride> overrides, String scope) {
...
private static FilterSettings upsertOverride(
    FilterSettings cur,
    String scopePattern,
    Boolean filtersEnabled, boolean filtersSpecified,
    Boolean placeholdersEnabled, boolean placeholdersSpecified,
    Boolean collapsed, boolean collapsedSpecified
) {
  String scope = (scopePattern == null || scopePattern.isBlank()) ? "*" : scopePattern.trim();
  ...
  if (!(nextOv.filtersEnabled() == null && nextOv.placeholdersEnabled() == null && nextOv.placeholdersCollapsed() == null)) {
    out.add(nextOv);
  }

  out.sort(Comparator.comparingInt((FilterScopeOverride o) -> o.scopePattern().length()).reversed());
  ...
  private static String toNormalizedAddCommand(FilterRule r) {
    ...
    if (r.hasTextRegex()) {
      String literal = "/" + r.textRegex().pattern().replace("/", "\\/") + "/" + flagsString(r.textRegex());
  ...
  private static String formatTokenValue(String v) {
    ...
    boolean needsQuotes = s.chars().anyMatch(Character::isWhitespace) || s.contains("\"") || s.contains("\\");
